...
class Classifier(nn.Module):
    ...
    def forward(
        ...
        fine_logits = self.linear_fine(x_fine.transpose(1, 2))  # (B, n_cls, T)
        fine_logits = fine_logits.transpose(1, 2)  # (B, T, n_cls)

        x_coarse = self.linear_coarse_1(x_coarse.transpose(1, 2))  # (B, D, T)
        x_coarse = self.dropout(x_coarse)
        coarse_logits = self.linear_coarse_2(x_coarse)  # (B, n_cls, T)
        coarse_logits = coarse_logits.transpose(1, 2)  # (B, T, n_cls)
